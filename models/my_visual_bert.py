...
class VisualBERTBase(BertPreTrainedModel):
    ...
    def forward(
        ...
        if (
            ...
            encoded_layers = self.encoder(embedding_output, extended_attention_mask, output_attentions=self.output_attentions, output_hidden_states=self.output_hidden_states)
            sequence_output = encoded_layers[0]
...
class VisualBERT(BaseModel):
    ...
    def config_path(cls):
        ...
        return "configs/models/my_visual_bert.yaml"
...
# class LanguageAndVisionConcat(VisualBERT):
